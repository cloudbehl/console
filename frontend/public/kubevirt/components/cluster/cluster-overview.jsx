...
const CONSUMERS_CPU_QUERY =
  'sort(topk(10, sum by (pod_name)(container_cpu_usage_seconds_total{pod_name!=""})))';
const CONSUMERS_MEMORY_QUERY =
  'sort(topk(10, sum by (pod_name)(container_memory_usage_bytes{pod_name!=""})))';
...
const CAPACITY_STORAGE_USED_DEFAULT_QUERY =
  'sum(node_filesystem_avail_bytes) - sum(node_filesystem_free_bytes)';

const CAPACITY_NETWORK_TOTAL_QUERY =
  'sum(avg by(instance)(node_network_speed_bytes))'; // TODO: needs to be refined
...
export class ClusterOverview extends React.Component {
  constructor(props) {
    ...
    this.setDetailsOpenshiftResponse = this._setDetailsOpenshiftResponse.bind(
      this
    );
  ...
  fetchHealth(callback) {
    coFetch(`${k8sBasePath}/healthz`)
      ...
      .then(text =>
        text === 'ok' && this._isMounted
          ? callback(true, 'All systems healthy')
          : callback(false, text)
      )
      .catch(() => {
        if (this._isMounted) {
  ...
  componentDidMount() {
    ...
    this.fetchPrometheusQuery(CONSUMERS_CPU_QUERY, response =>
      this.setConsumersData('cpu', 'CPU', response)
    );
    this.fetchPrometheusQuery(CONSUMERS_MEMORY_QUERY, response =>
      this.setConsumersData('memory', 'Memory', response)
    );

    this.fetchHealth(this.setHealthData);
    this.fetchPrometheusQuery(OPENSHIFT_VERSION_QUERY, response =>
      this.setDetailsOpenshiftResponse(response)
    );

    this.fetchPrometheusQuery(CAPACITY_CPU_TOTAL_QUERY, response =>
      this.setCapacityData('cpu', 'total', response)
    );
    this.fetchPrometheusQuery(CAPACITY_CPU_USED_QUERY, response =>
      this.setCapacityData('cpu', 'used', response)
    );
    this.fetchPrometheusQuery(CAPACITY_MEMORY_TOTAL_QUERY, response =>
      this.setCapacityData('memory', 'total', response)
    );
    this.fetchPrometheusQuery(CAPACITY_MEMORY_USED_QUERY, response =>
      this.setCapacityData('memory', 'used', response)
    );
    this.fetchPrometheusQuery(CAPACITY_STORAGE_TOTAL_QUERY, response =>
      this.setCapacityData('storage', 'total', response)
    );
    this.fetchPrometheusQuery(CAPACITY_STORAGE_USED_QUERY, response =>
      this.setCapacityData('storage', 'used', response)
    );
    this.fetchPrometheusQuery(CAPACITY_STORAGE_TOTAL_DEFAULT_QUERY, response =>
      this.setCapacityData('storage', 'totalDefault', response)
    );
    this.fetchPrometheusQuery(CAPACITY_STORAGE_USED_DEFAULT_QUERY, response =>
      this.setCapacityData('storage', 'usedDefault', response)
    );
    this.fetchPrometheusQuery(CAPACITY_NETWORK_TOTAL_QUERY, response =>
      this.setCapacityData('network', 'total', response)
    );
    this.fetchPrometheusQuery(CAPACITY_NETWORK_USED_QUERY, response =>
      this.setCapacityData('network', 'used', response)
    );
  }
